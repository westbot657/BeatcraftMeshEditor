...
void main() {
    ...
    float horizDist = length(localXZ);
    float dist = length(vec2(horizDist, camHeight));
    ...
    float halfSize = camHeight * 50.0;
    float fade = 1.0 - smoothstep(halfSize * 0.7, halfSize, horizDist);
    fragColor.a *= fade;

    float axisPixelWidth = pixelWidth * 0.8;
    float axisZ = abs(worldPos.z) / (fwidth(worldPos.z) + 1e-7);
    float axisX = abs(worldPos.x) / (fwidth(worldPos.x) + 1e-7);

    float xAxisAlpha = (1.0 - smoothstep(0.0, axisPixelWidth, axisZ)) * step(0.0, worldPos.x);
    float zAxisAlpha = (1.0 - smoothstep(0.0, axisPixelWidth, axisX)) * step(0.0, worldPos.z);
