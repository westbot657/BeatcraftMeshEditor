...
void main() {
    if (u_render_mode == 2) {
        ...
        vec4 vColor = vec4(0.0, 0.0, 0.2, 1.0);
        if (!gl_FrontFacing) {
        ...
        vec2 uv = (v_screen_uv.xy / (v_screen_uv.w * 2.0)) + 0.5;

        float distortionStrength = 0.001;
        vec3 noiseInput = u_worldPos;
        vec2 distortion = vec2(
            cnoise(noiseInput + vec3(23.1, 0.0, 0.0) + v_screen_uv.xyz),
            cnoise(noiseInput + vec3(0.0, 23.1, 0.0) + v_screen_uv.yxw)
        ) * distortionStrength;

        vec2 distortedUV = uv + distortion;

        vec4 color = texture(u_texture, distortedUV) / 2.5;

        vec4 bloomfog_color = texture(u_bloomfog, (v_screen_uv.xy/(v_screen_uv.z*4.0))+0.5);
        fragColor = lerpColor(color, bloomfog_color, abs(v_screen_uv.z));
    }
}
