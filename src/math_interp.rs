...
fn eval_expr(tokens: &mut PeekMatch<'_>, vars: &mut impl MapIndexable, is_degrees: bool) -> Option<f32> {
    let mut left = eval_term(tokens, vars, is_degrees)?;
    while let Some(token) = tokens.peek() {
        match token.as_str() {
            "+" => left += { tokens.next(); eval_term(tokens, vars, is_degrees)? },
            "-" => left -= { tokens.next(); eval_term(tokens, vars, is_degrees)? },
            _ => break,
        }
    }
...
fn eval_term(tokens: &mut PeekMatch<'_>, vars: &mut impl MapIndexable, is_degrees: bool) -> Option<f32> {
    let mut left = eval_factor(tokens, vars, is_degrees)?;
    while let Some(token) = tokens.peek() {
        match token.as_str() {
            "*" => left *= { tokens.next(); eval_factor(tokens, vars, is_degrees)? },
            "/" => left /= { tokens.next(); eval_factor(tokens, vars, is_degrees)? },
            "%" => left %= { tokens.next(); eval_factor(tokens, vars, is_degrees)? },
            _ => break,
        }
...
fn eval_factor(tokens: &mut PeekMatch<'_>, vars: &mut impl MapIndexable, is_degrees: bool) -> Option<f32> {
    if let Some(token) = tokens.peek() {
        ...
        match tk {
            "-" => { tokens.next()?; Some(-eval_factor(tokens, vars, is_degrees)?) }
            ...
            "pow"   => eval_fn(|args| Some(args.first()?.powf(*args.get(1)?)), tokens, vars, is_degrees),
            "sign"  => eval_fn(|args| Some(args.first()?.signum()),            tokens, vars, is_degrees),
            ...
            "rads"  => eval_fn(|args| Some(args.first()?.to_radians()),        tokens, vars, is_degrees),
            "degs"  => eval_fn(|args| Some(args.first()?.to_degrees()),        tokens, vars, is_degrees),
...
fn eval_fn<'h>(
    ...
    if let Some(t) = tokens.peek() && t.as_str() == "(" {
        ...
        let mut args = vec![
            eval_expr(tokens, vars, is_degrees)?
        ];
